...
public class JSpotBoard extends JPanel implements SpotBoard {
	...
	private static final Color DEFAULT_SPOT_COLOR = Color.GREEN;
	...
	public JSpotBoard(int width, int height) {
		...
		for (int y=0; y<height; y++) {
			for (int x=0; x<width; x++) {
				...
				_spots[x][y] = new JSpot(Color.WHITE, DEFAULT_SPOT_COLOR, DEFAULT_HIGHLIGHT_COLOR, this, x, y);
	...
	public Spot getSpotAt(int x, int y) {
		if (x < 0 || x >= getSpotWidth() || y < 0 || y >= getSpotHeight()) {
			throw new IllegalArgumentException("Illegal spot coordinates " + x + " for x and " + y + " for y.");
		}
